...
class Jalali {
    ...
    function strtotime($time, $now = null) {
        ...
        if (preg_match($reg, $time)) {
            ...
            return self::mktime(
                            $Time[0], $Time[1], $Time[2], $Date[1], $Date[2], $Date[0]
            );
        } else {
    ...
    public function dateTime($now = 'now') {
...
///end conver to number in persian
    ...
    function jgetdate($timestamp = "") {
        ...
        return array(0 => $timestamp, "seconds" => self::date("s", $timestamp), "minutes" =>
            self::date("i", $timestamp), "hours" => self::date("G", $timestamp), "mday" => self::date("j", $timestamp), "wday" => self::date("w", $timestamp), "mon" => self::date("n", $timestamp),
            "year" => self::date("Y", $timestamp), "yday" => self::days_of_year(self::date("m", $timestamp), self::date("d", $timestamp), self::date("Y", $timestamp)), "weekday" => self::date("l", $timestamp),
